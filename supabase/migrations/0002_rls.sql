...
-- 보통은 '내 주문만 보여주기'를 앱 코드가 한다. 조건 하나만 빠뜨려도 남의 주문이 다 보인다.
-- 여기서는 그 규칙을 데이터베이스에 못 박는다. 코드가 틀려도 데이터베이스가 걸러낸다.
-- 규칙 23개로 이런 것들을 보장한다:
--   · 상품은 누구나 본다. 하지만 고치는 건 그 상품 주인만.
--   · 주문은 산 사람과 판 사람만 본다.
--   · 판매자는 발송하려고 배송지는 보지만, 다른 판매자에게서 얼마 샀는지는 못 본다.
--   · 아무도 스스로 운영자가 될 수 없다.
-- ══════════════════════════════════════════════════════════════
